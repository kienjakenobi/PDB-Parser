...
module PRAromatics (mainH, sideH, getAromatics, getTrpAromatic, 
    hBondMFinder, hBondSFinder) where
...
-- Take a list of the atoms in an Phe or Tyr residue and return the Aromatic
getPheTyrAromatic :: Residue -> Maybe Aromatic
getPheTyrAromatic res = case (c1, c2, c3, c4) of
    (_:_, _:_, _:_, _:_) -> Just aro
    _ -> Nothing
    where
    -- Get atom positions to define lines and plane
    c1 = getAtomName ats "CG"
    c2 = getAtomName ats "CZ"
    c3 = getAtomName ats "CE1"
    c4 = getAtomName ats "CD2"
    c1p = atomPos $ head c1
    c2p = atomPos $ head c2
    c3p = atomPos $ head c3
    c4p = atomPos $ head c4
    ...
    ats = resAtoms res
    -- Get one atom to get res name and id
    a = head ats
    aro = Aromatic
        { aroResName = atomAmino a
        , aroResID = atomRes a
        , aroCenter = appLineLineInter (Line2P {lp1 = c1p, lp2 = c2p}) (Line2P {lp1 = c3p, lp2 = c4p})
        , aroPlane = Plane3P {pp1 = c1p, pp2 = c2p, pp3 = c3p}
        }

-- Take a list of the atoms in a Trp residue and return the Aromatic
getTrpAromatic :: Residue -> Maybe Aromatic
getTrpAromatic res = case (c1, c2, c3, c4) of 
    (_:_, _:_, _:_, _:_) -> Just aro
    _ -> Nothing
    where
        -- Get atom positions to define lines and plane
    c1 = getAtomName ats "CD2"
    c2 = getAtomName ats "CH2"
    c3 = getAtomName ats "CZ2"
    c4 = getAtomName ats "CE3"
    c1p = atomPos $ head $ getAtomName ats "CD2"
    c2p = atomPos $ head $ getAtomName ats "CH2"
    c3p = atomPos $ head $ getAtomName ats "CZ2"
    c4p = atomPos $ head $ getAtomName ats "CE3"
    -- Get the atoms from the residue
    ats = resAtoms res
    -- Get one atom to get res name and id
    a = head ats
...
-- Take a list of residues and return a list of all aromatics in the list
--  It considers only those residues which should have an aromatic
getAromatics :: [Residue] -> [Aromatic]
getAromatics [] = []
getAromatics (r:rs)
    -- Get aromatics for PHE and TYR
    | atomAmino p `elem` [phe, tyr] = case (getPheTyrAromatic r) of
        -- Add to list if something is returned, otherwise skip
        (Just x) -> [x] ++ getAromatics rs
        _ -> getAromatics rs
    -- Get aromatics for TRP
    | atomAmino p == trp = case (getTrpAromatic r) of
        -- Add to list if something is returned, otherwise skip
        (Just x) -> [x] ++ getAromatics rs
        _ -> getAromatics rs
    -- Pass everything which does not have any aromatics to get
    | otherwise = getAromatics rs where
    -- Get the first atom in first residue
    p = head $ resAtoms r
    tyr = B.pack [84, 89, 82]
    phe = B.pack [80, 72, 69]
    trp = B.pack [84, 82, 80]
...
hBondMFinder (r:rs) aro
    ...
    | newH /= Nothing && backboneNPos `dist` inter > newHPos `dist` inter &&
        r1 <= 6.0 && bR1 <= 7.0  = 
